  ...
  return (
    <div className="py-12 animate-fade-in-up">
      <div className="flex justify-between items-center mb-8 px-2">
        <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-[#FDFCF0]/30 heading-font">Recent Shifts</h3>
        {logs.length > 3 && (
            <button 
                onClick={() => setIsExpanded(!isExpanded)}
                className="text-xs font-bold uppercase tracking-widest text-[#A8FFB5]/60 hover:text-[#A8FFB5] transition-colors"
            >
        ...
        <AnimatePresence mode="popLayout">
          {displayLogs.map((log) => (
            <motion.div 
              ...
              className="p-6 bg-white/2 border border-white/5 rounded-[2rem] flex flex-col gap-3 relative group overflow-hidden"
            >
              ...
              <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest text-white/20 heading-font">
                      {new Date(log.timestamp).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                  </span>
              </div>

              <p className="text-base font-light italic text-[#FDFCF0]/80 leading-relaxed pr-10">
                  "Kindred observed a shift in {log.category} ({log.delta > 0 ? '+' : ''}{log.delta.toFixed(1)}) {log.context}"
              </p>
            </motion.div>
          ))}
        </AnimatePresence>
